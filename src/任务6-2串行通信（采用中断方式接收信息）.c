//程序：ex6_2.c
//功能：乙机接收及显示程序，采用中断方式实现，密码为935467
...
{
	static unsigned char i = 0; //设置i为静态变量，这样i值只在第一次进入函数时才初始化，后面再进入时会自动记住之前的值
	EA = 0;						//每次进入中断程序时就把中断关掉，这样保证每次只接收一帧的数据
	RI = 0;						//接收完成，RI由软件清零，以便接收下一帧数据
	receive[i] = SBUF;			//接收的第i个数据
	i++;						//记录进入中断的次数
	if (i == 6)
	{
		i = 0; //当接收了6次之后就清零
	}
	EA = 1; //打开中断允许位，以便接收下一帧数据
}