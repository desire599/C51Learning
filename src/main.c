...
#define HZ_CHR_HEIGHT 16 //汉字宽度

#define uchar unsigned char //定义uchar 表示unsigned char
...
void Out_Char(uchar x, uchar y, uchar i, bit mode) // x为列坐标，y为页坐标，在指定位置显示字符
    ...
    {
        if (mode == 1)
            cData = GB_16[i][k]; //从GB_16汉字数组中取对应的汉字字码，存入准备数据存储变量
        else
            cData = ~GB_16[i][k];
        if (k < HZ_CHR_WIDTH) //汉字上半部，即取该汉字前16列的字码
        {
            if ((x + k) < 64) //坐标列数x的值加上当前的扫描列数如果小于64列，则只需要打开左半屏
            {
