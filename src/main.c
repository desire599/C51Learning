...
unsigned char IIC_ReceiveByte()
{
    ...
    while (i--)
    {
        ...
        SCL = 1;    //将时钟线置为1，可以稳定SDA的数据，进行数据读取
        Delay4us(); //等待读取数据的完成
        //特别注意：总共8位数据，移位只能7次，不然数值会出错。
        //由于第一次执行时recevieData是0，忽略了1次，因此循环8次实际只移位了7次。
        receiveData = receiveData << 1;//如果放在if-else语句的后面，则会移位8次，这样返回值会出错。要先把存储接收数据的变量左移1位，以便存储下一个返回数据的位。从而实现逐个位来保存从SDA数据线读取的数据
        //下面是读取数据，共8位，需要8次
...
void display()
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        P2 = ~temp;        //先赋值，需要4次，对temp取反（由0000 0001变为1111 1110）后赋值给P2，此时只打开第一个数码管P2.0
        if (i == 3)
            DP = 1; //当第4个数码管显示时，则其小数点引脚DP点亮
        for (j = 0; j < 100; j++)
            ;       //软件空循环，延时约1ms
        temp <<= 1; //移位操作只需要3次，将w的变量左移一位，0000 0010。这样第二次循环时，P2=~temp=1111 1101，只打开第二个数码管P2.1
